...
public class TokenControllerTest {
    ...
    public void getToken() throws Exception {
        ...
        int status = this.mockMvc.perform(
                post("/oauth/token")
                        .header("Authorization", "Basic " + Base64.getEncoder().encodeToString("app:app".getBytes()))
                        .params(map)
        ...
        switch (status) {
            case HttpStatus.SC_OK:
                System.out.println("密码授权模式获取令牌---------------->成功（200）");
                break;
            case HttpStatus.SC_UNAUTHORIZED:
            	System.out.println("密码授权模式获取令牌---------------->失败（401---没有权限，请检查验证信息，账号是否存在、客户端信息）");
                break;
            case HttpStatus.SC_BAD_REQUEST:
            	System.out.println("密码授权模式获取令牌---------------->失败（400---请求失败，请检查密码是否正确）");
                break;
            default:
            	System.out.println("密码授权模式获取令牌---------------->失败（{}---未知结果）"+status);
                break;
        }
